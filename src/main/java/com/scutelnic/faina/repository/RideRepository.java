...
public interface RideRepository extends JpaRepository<Ride, Long> {
    ...
    @Query("SELECT r FROM Ride r WHERE r.isActive = true AND r.travelDate >= :currentDate ORDER BY r.travelDate ASC")
    List<Ride> findAllActiveRides(@Param("currentDate") LocalDateTime currentDate);
    
    @Query("SELECT r FROM Ride r WHERE r.isActive = true AND r.fromLocation LIKE %:fromLocation% AND r.toLocation LIKE %:toLocation% AND r.travelDate >= :travelDate ORDER BY r.travelDate ASC")
    ...
    @Query("SELECT r FROM Ride r WHERE r.isActive = true AND r.availableSeats >= :minSeats ORDER BY r.travelDate ASC")
